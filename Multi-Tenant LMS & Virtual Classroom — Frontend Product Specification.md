...
|---|---|---|
| **Student ID** | Generated by the platform, **read-only** — never typed by an admin. Format `SCHOOL/YY/NNNN`: the school's short code, the admission year's last two digits and the next number in that year's sequence. | `RSHS/26/0076` |
...
Columns: `first_name, last_name, gender, date_of_birth, admission_year, jhs_index_number, class, guardian_name, guardian_phone, email`. There is **no Student ID column** — Student IDs are generated on import (§22.1). JHS index numbers that lost their leading zeros in Excel (7–9 digits) are padded back to 10 digits with a warning.
